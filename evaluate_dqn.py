...
def evaluate_dqn(
    ...
    """
    Evaluate a saved DQN checkpoint on the CoolSync environment.
    """
    ...
    eval_csv_path = f"results/logs/dqn_eval_{scenario_name}_{suffix}.csv"
    eval_json_path = f"results/summaries/dqn_eval_{scenario_name}_{suffix}.json"

    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(
            f"Checkpoint not found: {checkpoint_path}. "
            "Train the DQN agent first."
        )
    ...
    for episode in range(1, episodes + 1):
        ...
        print(
            f"[DQN Evaluation] Episode {episode}/{episodes} | "
